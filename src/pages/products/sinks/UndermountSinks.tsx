...
const UndermountSinks = () => {
  ...
  const undermountProducts = products.filter((product) => product.filters?.mounting_type === 'undermount');

  // Extract unique brands and materials
  const brands = ['all', ...Array.from(new Set(undermountProducts.map((p) => p.brand).filter(Boolean))) as string[]];
  const materials = ['all', ...Array.from(new Set(undermountProducts.map((p) => p.filters?.material).filter(Boolean))) as string[]];
