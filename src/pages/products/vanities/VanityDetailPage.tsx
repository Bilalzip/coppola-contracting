...
const VanityDetailPage: React.FC = () => {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (!data) {
          ...
          setProduct(data);
          // Fetch related vanities (exclude current)
          supabase
            .from('products')
            .select('*')
            .eq('category', 'vanity')
  ...
  if (!slug || notFound) return <Navigate to="/products/vanities" replace />;
  ...
  if (!product) return <Navigate to="/products/vanities" replace />;

  // Convert specs from {label, value}[] to Record<string, string> for ProductDetailLayout
